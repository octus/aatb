    ...
    while true do
      ...
      if(IsNearBank() or IsNearATM()) then
        if (atBank == false) then
          TriggerEvent('chatMessage', "", {0, 255, 0}, "^0Appuyez sur 'Touche Action' (Défaut: E) pour ouvrir le menu");
        end
        atBank = true
        if IsControlJustPressed(1, 38)  then -- IF INPUT_PICKUP Is pressed
          if (IsInVehicle()) then
            TriggerEvent('chatMessage', "", {255, 0, 0}, "^1Vous ne pouvez pas utiliser la banque depuis un véhicule!");
          else
            if bankOpen then
              closeGui()
              bankOpen = false
...
AddEventHandler('bank:deposit', function(amount)
  if(IsNearBank() == true or depositAtATM == true and IsNearATM() == true or depositAnywhere == true ) then
    if (IsInVehicle()) then
      TriggerEvent('chatMessage', "", {255, 0, 0}, "^1Vous ne pouvez pas utiliser un ATM depuis un véhicule!");
    else
      TriggerServerEvent("bank:deposit", tonumber(amount))
    end
  else
    TriggerEvent('chatMessage', "", {255, 0, 0}, "^1Vous devez être dans une banque pour déposer votre argent!");
  end
end)
...
AddEventHandler('bank:withdraw', function(amount)
  if(IsNearATM() == true or IsNearBank() == true or withdrawAnywhere == true) then
    ...
    TriggerEvent('chatMessage', "", {255, 0, 0}, "^1Vous n'êtes pas dans une banque ou devant un ATM!");
  end
end)
...
AddEventHandler('bank:givecash', function(toPlayer, amount)
  if(IsNearPlayer(toPlayer) == true or giveCashAnywhere == true) then
    ...
    if (playing ~= false) then
      ...
      TriggerEvent('chatMessage', "", {255, 0, 0}, "^1Ce joueur n'est pas connecté!");
    end
  else
    TriggerEvent('chatMessage', "", {255, 0, 0}, "^1Vous n'êtes pas à proximité de ce joueur!");
  end
end)
